...
class FallDetectorWorker:
    ...
    def _rule_based_fall_check(self, track_id, bbox, kpts, frame_shape, model_prob, current_time):
        ...
        lying_by_box = aspect >= 1.35  # Cúi nhặt đồ box có thể rộng ra, nhưng nằm hẳn thì phải rộng hẳn
        ...
        # Mức điểm khả nghi 4.5 (phải thỏa mãn nhiều yếu tố hoặc yếu tố mạnh)
        ...
        if len(results) > 0 and results[0].boxes is not None and results[0].boxes.id is not None:
            ...
            if result.keypoints is not None:
                ...
                for i, track_id in enumerate(track_ids):
                    ...
                    w, h = x2 - x1, y2 - y1
                    if w < 30 or h < 30:  # bắt người nằm xa camera có box nhỏ
                        continue
